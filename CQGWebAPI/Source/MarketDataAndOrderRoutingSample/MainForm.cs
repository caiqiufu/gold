...
namespace MarketDataAndOrderRoutingSample
{
                         ...
                         IHistoricalDataConsumerInterface
   {
      ...
      private void fillOrderRow(OrderStatus orderStatus, int rowIndex, bool newOrder)
      {
         ...
         OrdersGridView[ColOrderPlaceTime.Index, rowIndex].Value = m_BaseTime.AddMilliseconds(orderStatus.submission_utc_time).ToString();
      ...
      private void showContractSpecification(int rowIndex, uint contractId)
      {
         ...
         if (isRowSubscribed(rowIndex))
         {
            if (metadata != null)
            {
               ...
               ContrSpecGridView[ColContSpecValue.Index, (int)ContractSpecs.FirstNoticeDate].Value = metadata.first_notice_date == 0 ? "" : m_BaseTime.AddMilliseconds((double)metadata.first_notice_date).ToShortDateString();
