      ...
      public void HCTimeAndSalesReportReceived(TimeAndSalesReport report)
      {
         if (isTicksReportStatusAcceptable(report.result_code) && m_TimeAndSallesRequestedIDsToContractID.Key == report.request_id && !m_UIClosing)
         {
            Invoke((MethodInvoker)delegate
            {
               ...
               TimeSpan utcStartTime = TimeSpan.FromMilliseconds(report.quote[0].quote_utc_time);
               foreach (Quote ticks in report.quote)
               {
                  try
                  {
                     ...
                     TicksDataGridView[colTSTimestamp.Index, rowIndex].Value = m_BaseTime.Add(utcStartTime).ToString("yyyy'-'MM'-'dd'T'HH':'mm':'ss.fff");
            ...
            Invoke((MethodInvoker)delegate
            {
               ...
               foreach (TimeBar tBar in report.time_bar)
               {
                  try
                  {
                     ...
                     TimeBarsDataGridView[ColTBClose.Index, rowIndex].Value = m_LastTimeBarsParams.bar_unit == (uint)TimeBarParameters.BarUnit.DAY ? getPriceValueStringOrNA(contractID, tBar.settlement_price) : getPriceValueStringOrNA(contractID, tBar.close_price);
