...
namespace RealTimeQuotes
{
	class Program
	{
		...
		private void Proxy()
		{
			var qc = new QuoteClient(8681572, "zy3ojco", "mt4-demopro-dc1.roboforex.com", 443, "95.217.201.27", 
		...
		void CloseBy()
		{
			try
			{
				//var r = Broker.Search("activtrades");
				//QuoteClient qc = new QuoteClient(83229126, "uz5rocb", "78.140.180.200", 443); //mq
				//QuoteClient qc = new QuoteClient(524870, "z8x247kp", "47.244.129.146", 443);
				//var qc = new QuoteClient(8681572, "zy3ojco", "mt4-demopro-dc1.roboforex.com", 443);
				var qc = new QuoteClient(7001, "abc123", "metaserver1.surya-am.com", 443);
		...
		{
			var qc = new QuoteClient(8223204, "Ploy0512", "t02dsg.exness.cloud", 443);
			qc.Connect();
			Thread.Sleep(1000);
		...
		private void CloseAllOrders()
{
	...
	foreach (var order in qc.GetOpenedOrders())
	{
		...
		private void OpenOrderOnQuote()
		{
			...
			//QuoteClient qc = new QuoteClient(8223204, "Ploy0511", "t02dsg.exness.cloud", 443);
			//QuoteClient qc = new QuoteClient(14110914, "Akapol55", "r04d00.exness.cn", 443);
			//QuoteClient qc = new QuoteClient(2090226925, "A4B8316", "95.168.188.22", 443);
			//QuoteClient qc = new QuoteClient(22356792, "TImurila", "mt4demo206.loginandtrade.com", 443);
			//var qc = new QuoteClient(2102309948, "4zhyzwn", "54.158.78.142", 443);
			QuoteClient qc = new QuoteClient(1511754, "ao4kbct", "37.58.56.234", 444);
			Console.WriteLine("Connecting...");
			qc.Connect();
	...
	private void GetDemo()
		{
			for (int i = 0; i < 1; i++)
			{
				var d = QuoteClient.GetDemo("88.212.244.84", 1951, 500, 2000000, "timurilasaf", "demobitcoin", "Canada"
				...
				File.AppendAllText("acc.txt", d.User + " " + d.Password + Environment.NewLine);
				Thread.Sleep(1000);
			}
		...
		{
			//var qc = new QuoteClient(6776747, "Wg6fb149ce", "213.175.208.159", 443);
			//var qc = new QuoteClient(2102309948, "4zhyzwn", "54.158.78.142", 443);
			//var qc = new QuoteClient(992495, "Test@123", "52.56.159.183", 443);
			QuoteClient qc = new QuoteClient(78300003, "apitest123", "demo.unitedglobalasset.com", 443);
			Console.WriteLine("Connecting");
			qc.Connect();
			Console.WriteLine("Connected");
		...
		{
			//QuoteClient qc = new QuoteClient(19029784, "x1bT1u8pA", "91.220.199.46", 443);
			QuoteClient qc = new QuoteClient(11369297, "iuyf13", "170.75.202.214", 443);
			//QuoteClient qc = new QuoteClient(1235301029, "czey3uq", "109.169.86.162", 443);
			Console.WriteLine("Connecting...");
			qc.Connect();
		...
		void AccountProfitCheck()
		{
			...
			//QuoteClient qc = new QuoteClient(2090226925, "A4B8316", "95.168.188.22", 443);
			//QuoteClient qc = new QuoteClient(22356792, "TImurila", "mt4demo206.loginandtrade.com", 443);
			//QuoteClient qc = new QuoteClient(2090405756, "G8mulIq", "62.171.147.215", 443);
			//QuoteClient qc = new QuoteClient(1280793, "ETSnjike202020202020", "dc1.mt4ecn.mydcnet.com", 443);
			var qc = new QuoteClient(3011599, "led369369", "main.trade.best-leader.com", 443);
			Console.WriteLine("Connecting...");
			qc.Connect();
			Console.WriteLine("Connected");
			var oc = new OrderClient(qc);
		...
		void Market()
		{
			...
			//QuoteClient qc = new QuoteClient(2090067187, "Aa88888888", "46.235.34.161", 443);
			//QuoteClient qc = new QuoteClient(19029784, "x1bT1u8pA", "91.220.199.46", 443);
			//var qc = new QuoteClient(8223204, "Ploy0511", "t02dsg.exness.cloud", 443);
			QuoteClient qc = new QuoteClient(8681572, "zy3ojco", "mt4-demopro-dc1.roboforex.com", 443);
			//QuoteClient qc = new QuoteClient(1511754, "ao4kbct", "37.58.56.234", 444); //tradersway demo
			//QuoteClient qc = new QuoteClient(78300003, "apitest123", "203.201.174.155", 443);
			//QuoteClient qc = new QuoteClient(2830804, "Maslina01!", "18.135.73.214", 443);
			Console.WriteLine("Connecting...");
			qc.Connect();
			string symbol = qc.Symbols[0];
			OrderClient oc = new OrderClient(qc);
		...
		{
			//QuoteClient qc = new QuoteClient(8681572, "zy3ojco", "mt4-demopro-dc1.roboforex.com", 443);
			QuoteClient qc = new QuoteClient(743, "50z#PZfP/BI8", "94.236.7.233", 443);
		...
		{
			//QuoteClient qc = new QuoteClient(8681572, "zy3ojco", "mt4-demopro-dc1.roboforex.com", 443);
			QuoteClient qc = new QuoteClient(61054681, "d98b65caS", "193.42.110.91", 443);
			//qc.GetSymbolInfo("").Ex.
			//qc.Connect();
		...
		void ModifyMarket()
		{
			...
			//QuoteClient qc = new QuoteClient(498667, "cca3esj", "demo-mt4c.ig.com", 443); //robo demo
			QuoteClient qc = new QuoteClient(465826, "abcd1234", "51.140.178.115", 443);
			Console.WriteLine("Connecting...");
			qc.Connect();
			var oc = new OrderClient(qc);
			Console.WriteLine("Connected to server");
...
private void Oc_OnOrderProgress(object sender, OrderProgressEventArgs args)
{
	Console.WriteLine(args.Type + " " + DateTime.Now.ToString("hh.mm.ss.ffffff"));
      ...
      while (qc.GetQuote(symbol).Ask == ask)
		...
		void OrderUpdate()
		{
			//QuoteClient qc = new QuoteClient(2316939, "qukNMb5X", "cndc1.gkfx.com", 4633);
			//QuoteClient qc = new QuoteClient(1235301029, "czey3uq", "109.169.86.162", 443);
			//QuoteClient qc = new QuoteClient(45159, "test123", "demo4london.evolve.markets", 1950);
			QuoteClient qc = new QuoteClient(61013955, "h0200Da6A", "193.42.110.91", 443);
			//QuoteClient qc = new QuoteClient(313894, "p4vkwij", "demo.mt4tickmill.eu", 443);
			//QuoteClient qc = new QuoteClient(8681572, "zy3ojco", "mt4-demopro-dc1.roboforex.com", 443);
			qc.OnOrderUpdate += Qc_OnOrderUpdate;
		...
		void Quotes2()
		{
			...
			//QuoteClient qc = new QuoteClient(61013955, "h0200Da6A", "185.10.45.25", 443)
			//var qc = new QuoteClient(8681572, "zy3ojco", "mt4-demopro-dc1.roboforex.com", 443);
			//QuoteClient qc = new QuoteClient(9195219, "Abc888888", "r07d13.exness.cloud", 443);
			//QuoteClient qc = new QuoteClient(24176690, "Tuktuk2020", "47.110.82.36", 443);
			//QuoteClient qc = new QuoteClient(5129144, "nDmJ2j@791", "185.96.244.18", 443);
			//QuoteClient qc = new QuoteClient(37020461, "dk395XFC", "t05d00.exness.cloud", 443);
			//QuoteClient qc = new QuoteClient(1118, "abc0099", "47.112.230.1", 443);
			//QuoteClient qc = new QuoteClient(97107157, "uoeh1rj", "78.140.180.200", 443);
			//QuoteClient qc = new QuoteClient(7293434, "qqqCPJ46761938", "13.212.22.99", 443);
			//QuoteClient qc = new QuoteClient(5140477, "Aa789789", "192.81.111.227", 443);
			QuoteClient qc = new QuoteClient(866266, "abc123", "39.105.51.83", 1950);
			Console.WriteLine("Connecting...");
			qc.Connect();
			Console.WriteLine("Connected");
		...
		void AccountProfit()
		{
			...
			//QuoteClient qc = new QuoteClient(1900176481, "x5672", "65.52.141.163", 444);
			//QuoteClient qc = new QuoteClient(61013955, "h0200Da6A", "185.10.45.25", 443);
			//QuoteClient qc = new QuoteClient(71429543, "h5duefv", "mt4d02.fxcorporate.com", 443);
			QuoteClient qc = new QuoteClient(45111321, "Bsolhs@782", "95.168.188.22", 443);
			Console.WriteLine("Connecting...");
			qc.Connect();
			Console.WriteLine("Connected");
...
string Symbol = "EURUSD";
		...
		void PendingWithExpiration()
		{
			try
			{
				//QuoteClient qc = new QuoteClient(55967934, "ov2osqv", "88.212.244.84", 1951);
				//QuoteClient qc = new QuoteClient(8475055, "ABcd1234", "dc-d01-06.mt4servers.net", 443);
				//var qc = new QuoteClient(336067, "euo2nbn", "demo-mt4a.ig.com", 443);
				//var qc = new QuoteClient(8223204, "Ploy0511", "t02dsg.exness.cloud", 443);
				//var qc = new QuoteClient(8681572, "zy3ojco", "mt4-demopro-dc1.roboforex.com", 443);
				//var qc = QuoteClient.GetQuoteClient(20077888, "asd123", "113.28.156.216", 443);
				var qc = new QuoteClient(23993883, "6bU3Nbqn78l0", "37.187.164.80", 443);
				Console.WriteLine("Connecting...");
				qc.Connect();
				OrderClient oc = new OrderClient(qc);
				Console.WriteLine("Connected to server");
		...
		void ModifyPending()
		{
			...
			//QuoteClient qc = new QuoteClient(8475055, "ABcd1234", "dc-d01-06.mt4servers.net", 443);
			//QuoteClient qc = new QuoteClient(8681572, "zy3ojco", "mt4-demopro-dc1.roboforex.com", 443);
			//QuoteClient qc = new QuoteClient(511510219, "7ukdzxi", "51.140.68.15", 443);
			QuoteClient qc = new QuoteClient(465826, "abcd1234", "51.140.178.115", 443);
		...
		{
			Logger.OnMsg += Logger_OnMsg;
			//var res = Broker.Search("fxdd");
			//QuoteClient qc = new QuoteClient(8681572, "zy3ojco", "mt4-demopro-dc1.roboforex.com", 443);
			//QuoteClient qc = new QuoteClient(6778299, "fe2lzad", "213.175.208.159", 443);
			//QuoteClient qc = new QuoteClient(2090067187, "Aa88888888", "46.235.34.161", 443);
			//QuoteClient qc = new QuoteClient(8223204, "Ploy0511", "t02dsg.exness.cloud", 443);
			//QuoteClient qc = new QuoteClient(30711562, "nsnt53", "52.59.155.191", 443);
			//QuoteClient qc = new QuoteClient(9195219, "Abc888888", "r07d13.exness.cloud", 443);
			//QuoteClient qc = new QuoteClient(11608692, "cryj66", "45.76.230.185", 443);
			//QuoteClient qc = new QuoteClient(502086064, "e574c86c", "106.14.78.185", 443);
			//QuoteClient qc = new QuoteClient(59650083, "exCr3Tx", "185.212.80.88", 587);
			//QuoteClient qc = new QuoteClient(1005907138, "Abc123", "103.253.115.94", 443);
			//QuoteClient qc = new QuoteClient(1117, "abc111", "103.75.3.168", 443);
			//QuoteClient qc = new QuoteClient(3001002, "jr123123", "mt3dc13.jrjrapp.com", 443);
			//QuoteClient qc = new QuoteClient(40039837, "6kyRIMh35wVb", "170.75.200.12", 443);
			//QuoteClient qc = new QuoteClient(333892, "p8jgdsx", "34.232.56.156", 443);
			QuoteClient qc = new QuoteClient(2039455, "pAJdttxYrp", "104.238.141.69", 443);
			qc.Connect();
			...
			for (int i = 0; i < 1000; i++)
			{
				Console.WriteLine(i + " Connected to server. Balance = " + qc.AccountBalance + " Equity = " + qc.AccountEquity);
				Thread.Sleep(500);			
			}
