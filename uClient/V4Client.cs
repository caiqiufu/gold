...
namespace V4
{
    public class V4Client
    {
        ...
        public Button btn;
        public Form form;
        public V4Client(DataGridView gvPositions)
        {
        ...
        public OrderProgressEvent OpenBuyOrder1(double price, double slippage, double lot)
        {
            ...
            IntPtr ret = default(IntPtr);
            RenderUI(() => {
                ret = OpenBuyOrder(price, -slippage, lot);
            });
        ...
        public OrderProgressEvent OpenSellOrder1(double price, double slippage, double lot)
        {
            ...
            IntPtr ret = default(IntPtr);
            RenderUI(() => {
                ret = OpenSellOrder(price, -slippage, lot);
            });
            string res = Marshal.PtrToStringAnsi(ret);
            OrderProgressEvent orderEvent = GenerateOrderProgressEvent(uClient.Comm.Enum.Open, res);
            orderEvent.SendTime = sendTime;
        ...
        public TradingData GetUpdatedPositionList(TradingData tradingData, decimal askPrice, decimal bidPrice)
        {
            if (tradingData!=null)
            {
                IList<uClient.Comm.Position> positions = tradingData.positions;
                if (positions != null && positions.Count > 0)
                {
                    string oldProfit = tradingData.Profit;
                    tradingData.Profit = "0";
                    foreach (Position order in positions)
                    {
                        order.CurrentPrice = uClient.Comm.Enum.Buy.Contains(order.BuySell) ? bidPrice : askPrice;
                        decimal points = uClient.Comm.Enum.Buy.Contains(order.BuySell) ? (order.CurrentPrice - order.OpenPrice) : (order.OpenPrice - order.CurrentPrice);
                        order.Profit = points * 100 * order.Lot;
                        tradingData.Profit = Convert.ToString(Convert.ToDecimal(tradingData.Profit) + Convert.ToDecimal(order.Profit));
                    }
                    tradingData.AccountEquity = Convert.ToString(Convert.ToDecimal(tradingData.AccountEquity) + Convert.ToDecimal(tradingData.Profit) - Convert.ToDecimal(oldProfit));
                    tradingData.AccountFreeMargin =Convert.ToString( Convert.ToDecimal(tradingData.AccountEquity) - Convert.ToDecimal(tradingData.AccountMargin));
                }
            }
            return tradingData;
        }
        public TradingData GetOpenedOrders(string res)
        {
 ...
 */
        ...
        public Quote GetQuote1()
        {
            ...
            if (res != "" && res.Contains(':'))
            {
                //paras[1] 对应界面Buy（高价）==Ask，paras[2] 对应界面Sell(低价) Bid
