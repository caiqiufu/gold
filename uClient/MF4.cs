    ...
    public class MF4 : PlatformInf
    {
        ...
        public override void Connect()
        {
            ...
            if (string.IsNullOrEmpty(_Broker.IP(_Account.Type)) || string.IsNullOrEmpty(_Broker.Port(_Account.Type)))
            {
                ...
                if (!_Client.Login(_Account.UserCode, _Account.Password, "Live".Equals(_Account.Type), pathIndex, out string message1, -1))
                {
        ...
        public void OnQuote(Client sender, QuoteUpdatedEventArgs e)
        {
            ...
            {
                if (string.Equals(e.Quote.Symbol, _TradeSymbol))
                {
        ...
        public virtual IList<M4.Common.Classes.Position> myGetPosition()
        {
        ...
        public override void UpdatePositionGrid()
        {
            ...
            if (_Quote != null && orders != null && orders.Count > 0 && _GvPositions != null)
            {
        ...
        public override void UpdateQuote()
        {
            if (!_BgwBroberQuote.CancellationPending)
            {
                if (isConnect())
                {
                    Utils.UpdateQuoteDisplay(_QuotaDisplayPanel, _QuoteData, _Quote.BidPrice, _Quote.AskPrice);
                }
                else
                {
        ...
        public override void OrderUpdate(bool _IsAutoOperationFlag, DateTime _SendTime, string _LockOrderId)
        {
            ...
            if (_SystemMessage != null)
            {
                ...
                Dictionary<string, object> dic = Utils.GetSystemMessageReceived(_SystemMessage);
