...
static char _StrDeCode[2000] = { 0 };
static char _Watchwords[100] = { 0 };
static char _ClientId[100] = { 0 };
static unsigned short int _SessionLen_Quote = 0;
...
/****************************************************************************
*   函数功能：获取标准UUID，含中划线70fe761d-ce3c-465e-a260-bb0422bfa670
*   @param 无
*
*   @return UUID
*****************************************************************************/
...
bool Connect()
{
	...
	//设置发送超时
	setsockopt(_SOCK_FD, SOL_SOCKET, SO_SNDTIMEO, (char*)&nNetTimeout, sizeof(int));
	//设置接收超时
	setsockopt(_SOCK_FD, SOL_SOCKET, SO_RCVTIMEO, (char*)&nNetTimeout, sizeof(int));
...
bool ConnectQuote()
{
	...
	int nNetTimeout = 2000;
	//设置发送超时
	setsockopt(_SOCK_FD_QUOTE, SOL_SOCKET, SO_SNDTIMEO, (char*)&nNetTimeout, sizeof(int));
	//设置接收超时
	setsockopt(_SOCK_FD_QUOTE, SOL_SOCKET, SO_RCVTIMEO, (char*)&nNetTimeout, sizeof(int));
...
double GetBidPrice()
{
	char bidReadBuffer[100] = { 0 };
	FILE* fd = fopen("d://dlllog.txt", "ab+");
	fputs("===GetBidPrice=============================================", fd);
	memset(bidReadBuffer, 0, 100);
	char* quoteStr = RecvAll2(bidReadBuffer, 0);
	fputs("\n", fd);
	fputs(quoteStr, fd);
	fclose(fd);
	...
	//return atof(bid);
	vector<string> v = split(quoteStr, ":");
	if (v.size() > 0)
	{
		double price;
		string2num(v[2], price);
		return price;
	}
	return 0;
}
...
double GetAskPrice()
{
	char askReadBuffer[100] = { 0 };
	FILE* fd = fopen("d://dlllog.txt", "ab+");
	fputs("===GetAskPrice=============================================", fd);
	memset(askReadBuffer, 0, 100);
	char* quoteStr = RecvAll2(askReadBuffer, 0);
	fputs("\n", fd);
	fputs(quoteStr, fd);
	fclose(fd);
...
{
	HeartPeer();
	char buyReadBuffer[2000] = { 0 };
	memset(buyReadBuffer, 0, 2000);
	char buyBuffer[2000] = { 0 };
	memset(buyBuffer, 0, 2000);
	try {
		if (!isConnect())
		{
			Connect();
		...
		myHeader2.nNum4 = 0x0000;
		memcpy(buyBuffer, &myHeader2, 8);
		memcpy(buyBuffer + 8, _SessionId, _SessionLen);
		memcpy(buyBuffer + 8 + _SessionLen, szBuy, strlen(szBuy));
		SSL_write(_SSL, &myHeader, sizeof(myHeader));
		SSL_write(_SSL, buyBuffer, myHeader.nNum1);
		char* pRead = readData(buyReadBuffer);
		DisConnect();
		//fputs("\n", fd);
		//fputs(pRead, fd);
		//fclose(fd);
...
{
	HeartPeer();
	char sellReadBuffer[2000] = { 0 };
	memset(sellReadBuffer, 0, 2000);
	char sellBuffer[2000] = { 0 };
	memset(sellBuffer, 0, 2000);
	try
	{
		if (!isConnect())
		{
		...
		myHeader2.nNum4 = 0x0000;
		memcpy(sellBuffer, &myHeader2, 8);
		memcpy(sellBuffer + 8, _SessionId, _SessionLen);
		memcpy(sellBuffer + 8 + _SessionLen, szSell, strlen(szSell));
		SSL_write(_SSL, &myHeader, sizeof(myHeader));
		SSL_write(_SSL, sellBuffer, myHeader.nNum1);
		char* pRead = readData(sellReadBuffer);
		DisConnect();
		//fputs("\n", fd);
		//fputs(pRead, fd);
		//fclose(fd);
...
{
	HeartPeer();
	char closeReadBuffer[2000] = { 0 };
	memset(closeReadBuffer, 0, 2000);
	char closeBuffer[2000] = { 0 };
	memset(closeBuffer, 0, 2000);
	if (!isConnect())
	{
		Connect();
	}
	...
	memcpy(closeBuffer, &myHeader2, 8);
	memcpy(closeBuffer + 8, _SessionId, _SessionLen);
	memcpy(closeBuffer + 8 + _SessionLen, szCloseAll, strlen(szCloseAll));
	SSL_write(_SSL, &myHeader, sizeof(myHeader));
	SSL_write(_SSL, closeBuffer, myHeader.nNum1);
	char* pRead = readData(closeReadBuffer);
	DisConnect();
	//fputs("\n", fd);
	//fputs(pRead, fd);
	//fclose(fd);
...
char* LoginQuote()
{
	...
	char instrumentAndQuotePolicyIDs[2000] = { 0 };
