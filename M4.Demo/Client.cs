...
namespace M4.Demo {
	...
	/// 账户更新事件参数
	/// </summary>
	public sealed class AccountUpdatedEventArgs : EventArgs {
		/// <summary>
		/// 账户信息
		/// </summary>
	...
	/// 系统消息事件参数
	/// </summary>
	public sealed class SystemMessageReceivedEventArgs : EventArgs {
		/// <summary>
		/// 系统消息
		/// </summary>
	...
	/// 仓位更新事件参数
	/// </summary>
	public sealed class PositionUpdatedEventArgs : EventArgs {
		/// <summary>
		/// 当前仓位
		/// </summary>
	...
	public sealed class Client : IDisposable {
		...
		/// 当前持有仓位列表
		/// </summary>
		public IList<Position> OpenPositions => UpdatePositions(Broker.OpenPositionsNew);
		...
		private void OnAccountUpdated(AccountInfo accountInfo) {
		...
		private void OnOpenPositionUpdated(Position position) {
		...
		private bool LoginCore(string userName, string password, bool isLive, out string message, int pathIndex = 0, string ip = null, int port = 0) {
		...
		public void PlaceOrder(Quote quote, TradeSide side, int slippage, decimal lot) {
			if (quote is null)
		...
		public void PlaceOrder(string symbol, TradeSide side, decimal price, decimal lot, TradeType orderType, TimeInForce timeInForce) {
		...
		private static Order CreateOrder(Instrument instrument, TradeSide tradeSide, TradeType tradeType, bool bsd, int slippage, int slOrderPips, int tpOrderPips) {
			var order = new Order {
		...
		/// <param name="stopTimeInForce">止损期限</param>
		/// <param name="limitTimeInForce">止盈期限</param>
		public void LiquidateOrder(Position position, decimal stopPrice, decimal limitPrice, decimal stopLot, decimal limitLot, TimeInForce stopTimeInForce, TimeInForce limitTimeInForce) {
			if (position is null)
				throw new ArgumentNullException(nameof(position));
		...
		private static decimal CalculatePriceToOpenTrade(Quote quote, TradeSide side, decimal lot) {
		...
		private static decimal CalculatePriceToCloseTrade(Quote quote, TradeSide side, decimal lot) {
			decimal askPrice = quote.AskPriceAdjusted;
			for (int i = 0; i < quote.OrderBook.Count; i++) {
				if (quote.OrderBook[i].Ask > 0m)
					askPrice = quote.OrderBook[i].Ask;
