...
namespace V4
{
    class V4Client
    {
                ...
                if (accountNode != null)
                {
                    ...
                    foreach (XmlNode transactionNode in transactionNodes)
                    {
                        ...
                        foreach (XmlNode orderNode in orderNodes)
                        {
                            ...
                            CloseOrder1(mprice, openPrice, lot, orderId, dt.ToString(), isBuy);
                        }
                    }
                }
            }
        ...
        /// <param name="lot"></param>
        /// <returns></returns>
        public void OpenSellOrder1()
        {
            //double ask = GetAskPrice();
            IntPtr ret = OpenSellOrder(ask, 0, 0.5);
            string res = Marshal.PtrToStringAnsi(ret).ToString();
            Console.WriteLine("OpenSellOrder1");
            Console.WriteLine(res);
        }
        /// <summary>
        /// 平仓
        ...
        /// <param name="slippage"></param>
        /// <returns></returns>
        public void CloseOrder1(double price, double openPrice, double lot, string orderId, string openTime, string isBuy)
        {
            IntPtr ret10 = CloseOrder(price, 0, openPrice, lot, orderId, openTime, isBuy);
            string res = Marshal.PtrToStringAnsi(ret10).ToString();
            Console.WriteLine("CloseOrder1");
            Console.WriteLine(res);
        }
        /// <summary>
        /// 订阅显示的行情
        ...
        {
            IntPtr ret8 = RecvData2();
            string res = Marshal.PtrToStringAnsi(ret8).ToString();
            Console.WriteLine(res);
            if (res != "" && res.Contains("19:"))
            {
                bid = Convert.ToDouble(res.Substring(3, 7));
                ask = Convert.ToDouble(res.Substring(11, 7));
            }
        }
        public void getTrading()
        {
            IntPtr ret8 = GetTradingData();
            string res = Marshal.PtrToStringAnsi(ret8).ToString();
            Console.WriteLine(res);
        }
    }
